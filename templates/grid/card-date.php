...
global $post;
?>
<div class="spk-blogposts-item post-grid-date spbd-boxed-shadow">
    <div class="spk-blogposts-bx main-box-background" style="background-color: <?php echo  $box_background_color;?>">
        ...
        <?php if($show_category == 'true') { ?>
            ...
            <?php } ?>
        <h2 class="spk-blogpost-title" style="color: <?php echo  $text_colorpicker;?>">
            <a href="<?php the_permalink(); ?>" target="_self" class="sp-text-color" ><?php echo get_the_title(); ?></a>
        </h2>
        <div class="spk-blogpost-i1nfo">
            <?php if($show_author == 'true') { ?>
                <span class="sp-text-color" style="color: <?php echo  $text_colorpicker;?>;fill:<?php echo  $text_colorpicker;?>">
                    ...
                    <a href="<?php echo esc_url( get_author_posts_url( $post->post_author ) ); ?>"><?php the_author(); ?></a>
                </span>
            <?php } ?>
            <?php if($show_date == "true") { ?>
                <span class="sp-text-color" style="color: <?php echo  $text_colorpicker;?>;fill:<?php echo  $text_colorpicker;?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 443.294 443.294">
        ...
        <?php if($show_content == "true") { ?>
            <?php if($words_limit > 0 ) { ?>
                <div class="spk-blogpost-rte sp-text-color" style="color: <?php echo  $text_colorpicker;?>"><?php echo spbd_get_post_excerpt( $post->ID, get_the_content(), $words_limit); ?>
                </div>
