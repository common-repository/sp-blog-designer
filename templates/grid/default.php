...
global $post;
?>
<div class="spk-blogposts-item post-grid-default">
	<div class="spk-blogposts-bx box-background" style="background-color: <?php echo $background_color?>">
		<?php if ( $feat_image ) { ?>
		...
		<div class="spk-blogposts-text">
			...
			<?php } ?>
			<h2 class="spk-blogpost-title" style="color: <?php echo $text_colorpicker;?>"><a href="<?php the_permalink(); ?>" class="sp-text-color"><?php echo get_the_title(); ?></a></h2>
			<?php if($show_date == "true" || $show_author == 'true') { ?>
			<div class="spk-blogpost-i1nfo">
				<?php if($show_author == 'true') { ?>
					<span class="sp-text-color" style="color: <?php echo $text_colorpicker;?>;fill:<?php echo $text_colorpicker;?>"><svg  class='spk-svg-color' xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 512 512"><path d="M437.02,330.98c-27.883-27.882-61.071-48.523-97.281-61.018C378.521,243.251,404,198.548,404,148    C404,66.393,337.607,0,256,0S108,66.393,108,148c0,50.548,25.479,95.251,64.262,121.962    c-36.21,12.495-69.398,33.136-97.281,61.018C26.629,379.333,0,443.62,0,512h40c0-119.103,96.897-216,216-216s216,96.897,216,216    h40C512,443.62,485.371,379.333,437.02,330.98z M256,256c-59.551,0-108-48.448-108-108S196.449,40,256,40    c59.551,0,108,48.448,108,108S315.551,256,256,256z"/></svg> <a href="<?php echo esc_url( get_author_posts_url( $post->post_author ) ); ?>" style="color: inherit;"><?php the_author(); ?></a></span>
				<?php } ?>
				<?php if($show_date == "true") { ?>
					<span class="sp-text-color"  style="color: <?php echo $text_colorpicker;?>;fill:<?php echo $text_colorpicker;?>"><svg class='spk-svg-color' xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 443.294 443.294"><path d="m221.647 0c-122.214 0-221.647 99.433-221.647 221.647s99.433 221.647 221.647 221.647 221.647-99.433 221.647-221.647-99.433-221.647-221.647-221.647zm0 415.588c-106.941 0-193.941-87-193.941-193.941s87-193.941 193.941-193.941 193.941 87 193.941 193.941-87 193.941-193.941 193.941z"/><path d="m235.5 83.118h-27.706v144.265l87.176 87.176 19.589-19.589-79.059-79.059z"/></svg> <?php echo get_the_date(); ?></span>
				<?php } ?>
			</div>
			<?php } ?>
